...
void ssd1306DrawXBM(int16_t x, int16_t y, uint16_t width, int16_t height, const uint8_t *xbm, int16_t color )
{
    int16_t widthInXbm = (width + 7) / 8;
    uint8_t data = 0;
    int16_t yy,xx;

    for(yy = 0; yy < height; yy++, y++) {
        for(xx = 0; xx < width; xx++ ) {
            if (xx & 7) {
                data >>= 1;
            } else {
                data = xbm[ (xx / 8) + yy * widthInXbm];
            }

            if (data & 0x01) {
                ssd1306DrawPixel( x+xx, y, color, LAYER0);
            }
        }
    }
}

void ssd1306DrawCircle(int16_t x0, int16_t y0, int16_t radius, uint16_t color, uint16_t layer) {
  int16_t x = 0, y = radius;
    ...
    do {
        ...
        ssd1306DrawPixel( x0 + x, y0 + y, color, layer); //setPixel(x0 + x, y0 + y);     //For the 8 octants
        ssd1306DrawPixel( x0 - x, y0 + y, color, layer); //setPixel(x0 - x, y0 + y);
        ssd1306DrawPixel( x0 + x, y0 - y, color, layer); //setPixel(x0 + x, y0 - y);
        ssd1306DrawPixel( x0 - x, y0 - y, color, layer); //setPixel(x0 - x, y0 - y);
        ssd1306DrawPixel( x0 + y, y0 + x, color, layer); //setPixel(x0 + y, y0 + x);
        ssd1306DrawPixel( x0 - y, y0 + x, color, layer); //setPixel(x0 - y, y0 + x);
        ssd1306DrawPixel( x0 + y, y0 - x, color, layer); //setPixel(x0 + y, y0 - x);
        ssd1306DrawPixel( x0 - y, y0 - x, color, layer); //setPixel(x0 - y, y0 - x);
  ...
  ssd1306DrawPixel( x0 + radius, y0, color, layer); //setPixel(x0 + radius, y0);
  ssd1306DrawPixel( x0, y0 + radius, color, layer); //setPixel(x0, y0 + radius);
  ssd1306DrawPixel( x0 - radius, y0, color, layer); //setPixel(x0 - radius, y0);
  ssd1306DrawPixel( x0, y0 - radius, color, layer); //setPixel(x0, y0 - radius);
}
